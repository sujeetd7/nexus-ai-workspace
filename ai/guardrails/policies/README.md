...
```json
{
  ...
  "guardrails": {
    "GUARDRAIL_PROMPT_INJECTION": { "enabled": true, "config": { "blockThreshold": 0.8 } },
    "GUARDRAIL_PII":               { "enabled": true, "config": { "mode": "MASK" } },
    "GUARDRAIL_CONTENT_MODERATION":{ "enabled": true, "config": { "flagThreshold": 0.8 } },
    "GUARDRAIL_RESPONSE_VALIDATION":{"enabled": true, "config": { "maxLengthChars": 100000 } }
  }
}
```
