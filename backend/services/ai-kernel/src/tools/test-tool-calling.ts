...
async function testToolCalling() {
  ...
  try {
    ...
    console.log(
      "Available tools:",
      toolRegistry.metadata().map((t) => t.name),
    );
    console.log(
      "Tool definitions:",
      JSON.stringify(toolRegistry.definitions(), null, 2),
    );
    ...
    if (executionModule) {
      ...
      console.log(
        "Tool-calling executor available:",
        !!executorRegistry.getExecutor("tool_calling"),
      );
    }
