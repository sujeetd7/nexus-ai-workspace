...
export interface IPluginLoader {
  ...
  load(pluginId: string, options?: PluginLoadOptions): Promise<PluginDescriptor>;
  unload(pluginId: string): Promise<boolean>;
  reload(pluginId: string, options?: PluginLoadOptions): Promise<PluginDescriptor>;
...
export class PluginLoader implements IPluginLoader {
  ...
  public async discover(options: PluginDiscoveryOptions): Promise<PluginDescriptor[]> {
  ...
  public async load(pluginId: string, options: PluginLoadOptions = {}): Promise<PluginDescriptor> {
    ...
    if (plugin.status === PluginStatus.LOADED || plugin.status === PluginStatus.ACTIVE) {
      if (!options.overwriteExisting) {
        throw new PluginStateException(pluginId, plugin.status, "unloaded", "Plugin is already loaded");
      }
    ...
    try {
      ...
      if (options.autoRegister !== false && this.agentRegistry) {
        try {
          const agentInstance = await plugin.factory.createAgent(options.config);
          ...
          this.warnings.push(`Plugin '${pluginId}' loaded but agent registration failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
      } else {
        await this.pluginRegistry.updateStatus(pluginId, PluginStatus.LOADED);
      }
    ...
    try {
      ...
      if (plugin.agentInstance && this.agentRegistry) {
        try {
          ...
          this.warnings.push(`Failed to unregister agent for plugin '${pluginId}': ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
      }
  ...
  public async reload(pluginId: string, options: PluginLoadOptions = {}): Promise<PluginDescriptor> {
    // Unload first
    await this.unload(pluginId);
  ...
  public async health(): Promise<PluginHealth> {
    ...
    return {
      ...
      recentWarnings: [...registryHealth.recentWarnings, ...this.warnings.slice(-5)],
      metadata: {
  ...
  private async discoverInPath(searchPath: string, options: PluginDiscoveryOptions, currentDepth: number = 0): Promise<PluginDescriptor[]> {
      ...
      if (stat.isFile()) {
        ...
        for (const entry of entries) {
          ...
          if (entry.isFile()) {
            ...
            // Recursive discovery
            const subPlugins = await this.discoverInPath(entryPath, options, currentDepth + 1);
            plugins.push(...subPlugins);
          } else if (entry.isSymbolicLink() && options.followSymlinks) {
            // Follow symlinks if enabled
            const realPath = await fs.realpath(entryPath);
            const subPlugins = await this.discoverInPath(realPath, options, currentDepth + 1);
            plugins.push(...subPlugins);
          }
        }
      }
    } catch (error) {
      this.warnings.push(`Failed to access path '${searchPath}': ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
    
    return plugins;
  }
  
  private async loadPluginFromFile(filePath: string): Promise<PluginDescriptor | null> {
    try {
      ...
      this.warnings.push(`Failed to load plugin from '${filePath}': ${error instanceof Error ? error.message : 'Unknown error'}`);
      return null;
    }
  }
  ...
  private async loadPluginExports(filePath: string): Promise<PluginExports> {
    try {
      ...
      if (!module.metadata || !module.capabilities || !module.factory) {
        throw new Error("Plugin must export metadata, capabilities, and factory");
      }
      ...
      throw new Error(`Failed to load plugin exports: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  
  private matchesPatterns(name: string, includePatterns?: string[], excludePatterns?: string[]): boolean {
