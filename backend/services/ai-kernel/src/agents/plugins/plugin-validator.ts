...
export interface IPluginValidator {
  ...
  validateMetadata(metadata: PluginMetadata): Promise<{ isValid: boolean; errors: string[]; warnings: string[]; }>;
  validateCapabilities(capabilities: PluginCapabilities): Promise<{ isValid: boolean; errors: string[]; warnings: string[]; }>;
  validateFactory(factory: PluginFactory): Promise<{ isValid: boolean; errors: string[]; warnings: string[]; }>;
  checkDuplicates(plugin: PluginDescriptor, existingPlugins: PluginDescriptor[]): Promise<{ duplicateId: boolean; duplicateName: boolean; duplicateCapabilities: string[]; }>;
}

export class PluginValidator implements IPluginValidator {
  
  public async validate(plugin: PluginDescriptor): Promise<PluginValidationResult> {
    const errors: string[] = [];
    ...
    const capabilitiesResult = await this.validateCapabilities(plugin.capabilities);
    ...
    try {
      ...
      configSchemaValid = false;
      errors.push(`Invalid configuration schema: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
    ...
    const status = isValid ? 
      (warnings.length > 0 ? PluginValidationStatus.WARNING : PluginValidationStatus.VALID) : 
  ...
  public async validateMetadata(metadata: PluginMetadata): Promise<{ isValid: boolean; errors: string[]; warnings: string[]; }> {
    const errors: string[] = [];
    const warnings: string[] = [];
    ...
    if (metadata.homepage && (typeof metadata.homepage !== 'string' || !this.isValidUrl(metadata.homepage))) {
  ...
  public async validateCapabilities(capabilities: PluginCapabilities): Promise<{ isValid: boolean; errors: string[]; warnings: string[]; }> {
    const errors: string[] = [];
    const warnings: string[] = [];
    
    // Validate capabilities array
    if (!capabilities.capabilities || !Array.isArray(capabilities.capabilities)) {
      ...
      capabilities.capabilities.forEach((capability, index) => {
        ...
        if (!capability.description || typeof capability.description !== 'string') {
          errors.push(`Capability at index ${index} must have a valid description`);
        }
      });
    }
    
    // Validate supported types
    if (!capabilities.supportedInputTypes || !Array.isArray(capabilities.supportedInputTypes)) {
    ...
    if (!capabilities.supportedOutputTypes || !Array.isArray(capabilities.supportedOutputTypes)) {
    ...
    if (capabilities.maxConcurrentExecutions !== undefined) {
      if (typeof capabilities.maxConcurrentExecutions !== 'number' || capabilities.maxConcurrentExecutions < 1) {
        errors.push("Max concurrent executions must be a positive number");
      }
    }
    
    if (capabilities.timeoutMs !== undefined) {
      if (typeof capabilities.timeoutMs !== 'number' || capabilities.timeoutMs < 0) {
        errors.push("Timeout must be a non-negative number");
      }
    }
    
    if (capabilities.retryAttempts !== undefined) {
      if (typeof capabilities.retryAttempts !== 'number' || capabilities.retryAttempts < 0) {
  ...
  public async validateFactory(factory: PluginFactory): Promise<{ isValid: boolean; errors: string[]; warnings: string[]; }> {
    const errors: string[] = [];
    const warnings: string[] = [];
    ...
    if (factory.validateConfig && typeof factory.validateConfig !== 'function') {
      warnings.push("Plugin factory validateConfig should be a function");
    }
    
    if (factory.getDefaultConfig && typeof factory.getDefaultConfig !== 'function') {
    ...
    if (factory.getDefaultConfig) {
      try {
        ...
        warnings.push(`Factory getDefaultConfig threw an error: ${error instanceof Error ? error.message : 'Unknown error'}`);
      }
    }
  ...
  public async checkDuplicates(plugin: PluginDescriptor, existingPlugins: PluginDescriptor[]): Promise<{ duplicateId: boolean; duplicateName: boolean; duplicateCapabilities: string[]; }> {
    ...
    for (const existing of existingPlugins) {
      ...
      if (existing.id === plugin.id && existing.sourcePath === plugin.sourcePath) {
        continue;
      }
      ...
      const existingCapabilityNames = existing.capabilities.capabilities.map(cap => cap.name);
      const pluginCapabilityNames = plugin.capabilities.capabilities.map(cap => cap.name);
      
      for (const capName of pluginCapabilityNames) {
        if (existingCapabilityNames.includes(capName) && !duplicateCapabilities.includes(capName)) {
