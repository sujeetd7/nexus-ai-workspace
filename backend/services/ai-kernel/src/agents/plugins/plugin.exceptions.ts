export class PluginException extends Error {
  ...
  constructor(operation: string, pluginId?: string, message?: string) {
    super(message || `Plugin operation '${operation}' failed${pluginId ? ` for plugin '${pluginId}'` : ''}`);
...
export class PluginLoadException extends Error {
  ...
  constructor(pluginId: string, sourcePath: string, reason: string, message?: string) {
    super(message || `Failed to load plugin '${pluginId}' from '${sourcePath}': ${reason}`);
    this.pluginId = pluginId;
...
export class PluginValidationException extends Error {
  ...
  constructor(pluginId: string, errors: string[], warnings: string[] = [], message?: string) {
...
export class DuplicatePluginException extends Error {
  ...
  constructor(pluginId: string, conflictType: "id" | "name" | "capability", existingPlugin: string, message?: string) {
    super(message || `Duplicate plugin ${conflictType} '${pluginId}' conflicts with existing plugin '${existingPlugin}'`);
    this.pluginId = pluginId;
...
export class PluginStateException extends Error {
  ...
  constructor(pluginId: string, currentState: string, expectedState: string, message?: string) {
    super(message || `Plugin '${pluginId}' is in state '${currentState}' but expected '${expectedState}'`);
    this.pluginId = pluginId;
