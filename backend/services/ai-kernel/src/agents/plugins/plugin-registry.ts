...
export interface IPluginRegistry {
  ...
  getUsageStats(pluginId: string): Promise<{ usageCount: number; lastUsed?: Date; averageLoadTime?: number; }>;
...
export class PluginRegistry implements IPluginRegistry {
  ...
  public async register(plugin: PluginDescriptor): Promise<void> {
    ...
    if (this.plugins.has(plugin.id)) {
      throw new DuplicatePluginException(plugin.id, "id", plugin.id, "Plugin with this ID already exists");
    }
    
    // Check for duplicate name
    if (this.pluginsByName.has(plugin.name)) {
      const existingId = this.pluginsByName.get(plugin.name);
      throw new DuplicatePluginException(plugin.name, "name", existingId || "unknown", "Plugin with this name already exists");
    }
    
    // Validate plugin before registration
    const validationResult = await this.validator.validate(plugin);
    if (!validationResult.isValid) {
      throw new DuplicatePluginException(plugin.id, "id", plugin.id, `Plugin validation failed: ${validationResult.errors.join(", ")}`);
    }
    
    // Check for capability duplicates (warnings only)
    const duplicateCheck = await this.validator.checkDuplicates(plugin, Array.from(this.plugins.values()));
    if (duplicateCheck.duplicateCapabilities.length > 0) {
      this.warnings.push(`Plugin '${plugin.id}' has duplicate capabilities: ${duplicateCheck.duplicateCapabilities.join(", ")}`);
    }
  ...
  public async unregister(pluginId: string): Promise<boolean> {
    ...
    if (plugin.status === PluginStatus.ACTIVE) {
      throw new PluginStateException(pluginId, plugin.status, "inactive", "Cannot unregister active plugin");
    }
  ...
  public async clear(): Promise<void> {
    ...
    const activePlugins = Array.from(this.plugins.values()).filter(p => p.status === PluginStatus.ACTIVE);
    if (activePlugins.length > 0) {
      throw new PluginStateException("registry", "has-active-plugins", "no-active-plugins", `Cannot clear registry with ${activePlugins.length} active plugins`);
    }
  ...
  public async updateStatus(pluginId: string, status: PluginStatus): Promise<void> {
    const plugin = this.plugins.get(pluginId);
    if (!plugin) {
    ...
    if (status === PluginStatus.LOADED && previousStatus !== PluginStatus.LOADED) {
      plugin.loadedAt = new Date();
    }
    
    if (status === PluginStatus.ACTIVE && previousStatus !== PluginStatus.ACTIVE) {
  ...
  public async health(): Promise<PluginHealth> {
    ...
    const loadedPlugins = allPlugins.filter(p => p.status === PluginStatus.LOADED || p.status === PluginStatus.ACTIVE).length;
    const activePlugins = allPlugins.filter(p => p.status === PluginStatus.ACTIVE).length;
    const failedPlugins = allPlugins.filter(p => p.status === PluginStatus.FAILED).length;
    
    // Calculate performance metrics
    const pluginsWithLoadTime = allPlugins.filter(p => p.loadTime !== undefined);
    const averageLoadTime = pluginsWithLoadTime.length > 0 
      ? pluginsWithLoadTime.reduce((sum, p) => sum + (p.loadTime || 0), 0) / pluginsWithLoadTime.length
      : 0;
    
    const totalUsageCount = allPlugins.reduce((sum, p) => sum + p.usageCount, 0);
    ...
    if (failedPlugins > 0 || recentErrors.length > 0) {
      status = "unhealthy";
    } else if (recentWarnings.length > 0 || (totalPlugins > 0 && activePlugins === 0)) {
      status = "degraded";
    }
  ...
  public async getUsageStats(pluginId: string): Promise<{ usageCount: number; lastUsed?: Date; averageLoadTime?: number; }> {
    const plugin = this.plugins.get(pluginId);
    if (!plugin) {
      throw new PluginNotFoundException(pluginId);
    }
