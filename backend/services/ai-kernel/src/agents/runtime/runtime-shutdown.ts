...
export class RuntimeShutdownException extends Error {
  ...
  constructor(failedComponents: string[], message?: string) {
    super(message || `Failed to shutdown components: ${failedComponents.join(', ')}`);
...
export class RuntimeShutdownManager {
  ...
  public async shutdown(components: RuntimeComponents): Promise<ShutdownResult> {
    ...
    try {
      ...
      await this.shutdownComponent("BuiltinAgents", async () => {
        if (components.builtinRegistration.registry) {
          const allAgents = components.builtinRegistration.registry.listAllAgents();
          for (const instance of allAgents) {
            try {
              await instance.agent.shutdown();
              await components.agentRegistry.remove(instance.agent.metadata.id);
              components.builtinRegistration.registry.unregisterAgent(instance.id);
            } catch (error) {
              // Log but continue with other agents
              console.warn(`Failed to shutdown builtin agent ${instance.id}:`, error);
            }
          }
        }
      }, shutdownOrder, failedShutdowns);

      // 2. Shutdown orchestrator (highest level component)
      await this.shutdownComponent("Orchestrator", async () => {
        await components.orchestrator.stop();
      }, shutdownOrder, failedShutdowns);

      // 3. Shutdown coordinator
      await this.shutdownComponent("Coordinator", async () => {
      ...
      await this.shutdownComponent("LifecycleManager", async () => {
        ...
        for (const agent of agents) {
          try {
            ...
            // Log but continue with other agents
            console.warn(`Failed to shutdown agent lifecycle ${agent.metadata.id}:`, error);
          }
        }
      }, shutdownOrder, failedShutdowns);

      // 7. Shutdown communication manager
      await this.shutdownComponent("CommunicationManager", async () => {
      ...
      await this.shutdownComponent("PluginLoader", async () => {
      ...
      await this.shutdownComponent("Memory", async () => {
        ...
        console.log("Memory cleanup skipped - requires context-specific clearing");
      }, shutdownOrder, failedShutdowns);

      // 10. Clear agent registry (last)
      await this.shutdownComponent("AgentRegistry", async () => {
        const agents = await components.agentRegistry.list();
        for (const agent of agents) {
          try {
            ...
            // Log but continue
            console.warn(`Failed to remove agent ${agent.metadata.id}:`, error);
          }
        }
      }, shutdownOrder, failedShutdowns);
  ...
  private async shutdownComponent(
    ...
    try {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Unknown shutdown error';
