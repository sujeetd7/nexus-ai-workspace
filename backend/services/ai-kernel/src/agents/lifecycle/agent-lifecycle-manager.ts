...
export class AgentLifecycleManager implements IAgentLifecycleManager {
  ...
  private readonly validTransitions: Map<AgentLifecycleState, AgentLifecycleState[]> = new Map([
    [AgentLifecycleState.REGISTERED, [AgentLifecycleState.INITIALIZING]],
    [AgentLifecycleState.INITIALIZING, [AgentLifecycleState.READY, AgentLifecycleState.FAILED]],
    [AgentLifecycleState.READY, [AgentLifecycleState.RUNNING, AgentLifecycleState.STOPPING]],
    [AgentLifecycleState.RUNNING, [AgentLifecycleState.PAUSED, AgentLifecycleState.STOPPING]],
    [AgentLifecycleState.PAUSED, [AgentLifecycleState.RUNNING, AgentLifecycleState.STOPPING]],
  ...
  public async initialize(agentId: string): Promise<void> {
    ...
    if (!this.isValidTransition(currentState, AgentLifecycleState.INITIALIZING)) {
      throw new InvalidStateTransitionException(agentId, currentState, AgentLifecycleState.INITIALIZING);
    }
  ...
  public async start(agentId: string): Promise<void> {
    ...
    if (!this.isValidTransition(currentState, AgentLifecycleState.RUNNING)) {
      throw new InvalidStateTransitionException(agentId, currentState, AgentLifecycleState.RUNNING);
    }
    ...
    try {
      ...
      // Update agent status
      await agent.updateStatus(currentState === AgentLifecycleState.PAUSED ? 
  ...
  public async stop(agentId: string): Promise<void> {
    ...
    if (!this.isValidTransition(currentState, AgentLifecycleState.STOPPING)) {
      throw new InvalidStateTransitionException(agentId, currentState, AgentLifecycleState.STOPPING);
    }
  ...
  public async pause(agentId: string): Promise<void> {
    ...
    if (!this.isValidTransition(currentState, AgentLifecycleState.PAUSED)) {
      throw new InvalidStateTransitionException(agentId, currentState, AgentLifecycleState.PAUSED);
    }
  ...
  public async restart(agentId: string): Promise<void> {
    try {
      // Stop if running
      const currentState = this.getAgentState(agentId);
      if (currentState === AgentLifecycleState.RUNNING || currentState === AgentLifecycleState.PAUSED) {
  ...
  private isValidTransition(fromState: AgentLifecycleState, toState: AgentLifecycleState): boolean {
