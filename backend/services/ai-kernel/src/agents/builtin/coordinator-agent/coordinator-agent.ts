...
export class CoordinatorAgent implements IAgent {
  ...
  // Metrics
  private readonly operationCounts: Record<CoordinatorOperation, number> =
    {} as Record<CoordinatorOperation, number>;
  private readonly successCounts: Record<CoordinatorOperation, number> =
    {} as Record<CoordinatorOperation, number>;
  private readonly errorCounts: Record<CoordinatorOperation, number> =
    {} as Record<CoordinatorOperation, number>;
  private readonly latencies: Record<CoordinatorOperation, number[]> =
    {} as Record<CoordinatorOperation, number[]>;
  ...
  constructor(components: CoordinatorAgentComponents) {
    this.metadata = {
      ...
      description:
        "Built-in agent for multi-agent coordination operations using the existing AgentCoordinator infrastructure",
      version: "1.0.0",
    ...
    this.capabilities = [
      {
        ...
        description:
          "Assign coordination tasks to multiple agents with strategy-based execution",
      ...
      },
      {
        ...
        description:
          "Delegate individual tasks to specific agents with context propagation",
        inputSchema: {
          operation: "string",
          agentId: "string",
          task: "object",
      ...
      },
      {
        ...
        description:
          "Control and monitor coordination processes with collect and cancel operations",
    ...
    try {
      ...
      switch (request.operation) {
        case CoordinatorOperation.ASSIGN:
          result = await this.assignCoordination(
            request as CoordinatorAssignRequest,
            context,
          );
          break;
        case CoordinatorOperation.DELEGATE:
          result = await this.delegateTask(
            request as CoordinatorDelegateRequest,
            context,
          );
          break;
        case CoordinatorOperation.HANDOFF:
          result = await this.handoffAgent(
            request as CoordinatorHandoffRequest,
            context,
          );
          break;
        case CoordinatorOperation.BROADCAST:
          result = await this.broadcastMessage(
            request as CoordinatorBroadcastRequest,
            context,
          );
          break;
        case CoordinatorOperation.COLLECT:
          result = await this.collectResults(
            request as CoordinatorCollectRequest,
            context,
          );
          break;
        case CoordinatorOperation.CANCEL:
          result = await this.cancelCoordination(
            request as CoordinatorCancelRequest,
            context,
          );
          break;
        default:
          throw new InvalidCoordinatorOperationException(
      ...
      if (input && typeof input === "object" && "operation" in input) {
        this.recordOperationError(
          input.operation as CoordinatorOperation,
          duration,
        );
      }
  ...
  public async assignCoordination(
    request: CoordinatorAssignRequest,
    context: IAgentExecutionContext,
  ): Promise<CoordinatorAssignResult> {
    ...
    try {
      ...
      const result = await this.components.agentCoordinator.assign(
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Unknown coordination assignment error";
      throw new CoordinatorAssignmentException(
        request.coordinationRequest.agentIds,
        errorMsg,
      );
    }
  }
  ...
  public async delegateTask(
    request: CoordinatorDelegateRequest,
    context: IAgentExecutionContext,
  ): Promise<CoordinatorDelegateResult> {
  ...
  public async handoffAgent(
    ...
    try {
      ...
      const result = await this.components.agentCoordinator.handoff(
      ...
      const errorMsg =
        error instanceof Error ? error.message : "Unknown agent handoff error";
  ...
  public async broadcastMessage(
    request: CoordinatorBroadcastRequest,
    context: IAgentExecutionContext,
  ): Promise<CoordinatorBroadcastResult> {
    ...
    try {
      ...
      const result = await this.components.agentCoordinator.broadcast(
      ...
      return {
        ...
        metadata: {
          recipientCount: request.broadcastRequest.agentIds.length,
          messageType:
            typeof request.broadcastRequest.message === "object" &&
            request.broadcastRequest.message !== null &&
            "type" in request.broadcastRequest.message
      ...
      throw new CoordinatorBroadcastException(
        request.broadcastRequest.agentIds.length,
        errorMsg,
      );
    }
  }

  public async collectResults(
    request: CoordinatorCollectRequest,
    context: IAgentExecutionContext,
  ): Promise<CoordinatorCollectResult> {
    const collectedAt = new Date();

    try {
      // Collect using existing coordinator
      const result = await this.components.agentCoordinator.collect(
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Unknown result collection error";
      throw new CoordinatorCollectionException(
        request.coordinationId,
        errorMsg,
      );
    }
  ...
  public async cancelCoordination(
    ...
    try {
      ...
      const cancelled = await this.components.agentCoordinator.cancel(
        request.coordinationId,
      );
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Unknown coordination cancellation error";
      throw new CoordinatorCancellationException(
        request.coordinationId,
        errorMsg,
      );
    }
    ...
    try {
      ...
      return {
        ...
        completedCoordinations:
          this.successCounts[CoordinatorOperation.ASSIGN] || 0,
  ...
  public getMetrics(): CoordinatorAgentMetrics {
    ...
    const averageLatencies: Record<CoordinatorOperation, number> = {} as Record<
      CoordinatorOperation,
      number
    >;
    ...
    const successfulCoordinations =
      this.successCounts[CoordinatorOperation.ASSIGN] || 0;
    const failedCoordinations =
      this.errorCounts[CoordinatorOperation.ASSIGN] || 0;
    ...
    return {
      ...
      coordinationStats: {
        ...
        averageCoordinationTime:
          averageLatencies[CoordinatorOperation.ASSIGN] || 0,
      ...
      operationStats: {
        ...
        averageAgentsPerCoordination:
          this.totalCoordinations > 0
            ? this.agentParticipations / this.totalCoordinations
            : 0,
        delegationSuccessRate: this.calculateOperationSuccessRate(
          CoordinatorOperation.DELEGATE,
        ),
        handoffSuccessRate: this.calculateOperationSuccessRate(
          CoordinatorOperation.HANDOFF,
        ),
        broadcastSuccessRate: this.calculateOperationSuccessRate(
          CoordinatorOperation.BROADCAST,
        ),
      },
    };
  }
  ...
  private validateAndParseRequest(input: unknown): CoordinatorOperationRequest {
    ...
    if (
      !Object.values(CoordinatorOperation).includes(
        request.operation as CoordinatorOperation,
      )
    ) {
      throw new InvalidCoordinatorOperationException(
        request.operation as string,
    ...
    if (operation === CoordinatorOperation.ASSIGN) {
      if (
        !request.coordinationRequest ||
        typeof request.coordinationRequest !== "object"
      ) {
        throw new InvalidCoordinatorOperationException(
          operation,
          "Coordination request is required for assign operation",
        );
      }
    }
    ...
    if (operation === CoordinatorOperation.DELEGATE) {
      if (!request.agentId || typeof request.agentId !== "string") {
        throw new InvalidCoordinatorOperationException(
          operation,
          "Agent ID is required for delegate operation",
        );
      }
      if (request.task === undefined) {
        throw new InvalidCoordinatorOperationException(
          operation,
          "Task is required for delegate operation",
        );
      }
      if (!request.context || typeof request.context !== "object") {
        throw new InvalidCoordinatorOperationException(
          operation,
          "Context is required for delegate operation",
        );
      }
    }
    ...
    if (operation === CoordinatorOperation.HANDOFF) {
      if (
        !request.handoffRequest ||
        typeof request.handoffRequest !== "object"
      ) {
        throw new InvalidCoordinatorOperationException(
          operation,
          "Handoff request is required for handoff operation",
        );
      }
    }
    ...
    if (operation === CoordinatorOperation.BROADCAST) {
      if (
        !request.broadcastRequest ||
        typeof request.broadcastRequest !== "object"
      ) {
        throw new InvalidCoordinatorOperationException(
          operation,
          "Broadcast request is required for broadcast operation",
        );
      }
    }

    if (
      [CoordinatorOperation.COLLECT, CoordinatorOperation.CANCEL].includes(
        operation,
      )
    ) {
      if (
        !request.coordinationId ||
        typeof request.coordinationId !== "string"
      ) {
        throw new InvalidCoordinatorOperationException(
          operation,
          "Coordination ID is required for control operations",
        );
      }
    }
  ...
  private validateCoordinationRequest(request: any): void {
    ...
    const missingFields = requiredFields.filter(
      (field) => !(field in context) || !context[field as keyof typeof context],
    );
