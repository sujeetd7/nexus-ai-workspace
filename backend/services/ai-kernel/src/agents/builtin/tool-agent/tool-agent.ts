...
export class ToolAgent implements IAgent {
  ...
  // Metrics
  private readonly operationCounts: Record<ToolOperation, number> =
    {} as Record<ToolOperation, number>;
  private readonly successCounts: Record<ToolOperation, number> = {} as Record<
    ToolOperation,
    number
  >;
  private readonly errorCounts: Record<ToolOperation, number> = {} as Record<
    ToolOperation,
    number
  >;
  private readonly latencies: Record<ToolOperation, number[]> = {} as Record<
    ToolOperation,
    number[]
  >;
  private readonly toolUsage: Map<
    string,
    {
      executions: number;
      successes: number;
  ...
  constructor(components: ToolAgentComponents) {
    this.metadata = {
      ...
      name: "Tool Agent",
      description:
        "Built-in agent for tool discovery, execution, and batch operations using the existing Tool Registry and EnhancedToolExecutor",
      version: "1.0.0",
    ...
    try {
      ...
      this.agentHealth = {
        ...
        metrics: {
          ...
          totalExecutions: Object.values(this.operationCounts).reduce(
            (sum, count) => sum + count,
            0,
          ),
    ...
    try {
      ...
      switch (request.operation) {
        case ToolOperation.DISCOVER:
          result = await this.discoverTools(
            request as ToolDiscoverRequest,
            context,
          );
          break;
        ...
        case ToolOperation.EXECUTE:
          result = await this.executeTool(
            request as ToolExecuteRequest,
            context,
          );
          break;
        case ToolOperation.EXECUTE_BATCH:
          result = await this.executeBatch(
            request as ToolBatchExecuteRequest,
            context,
          );
          break;
        case ToolOperation.VALIDATE:
          result = await this.validateInput(
            request as ToolValidateRequest,
            context,
          );
          break;
        default:
          throw new InvalidToolOperationException(
  ...
  public async findTool(
    request: ToolFindRequest,
    context: IAgentExecutionContext,
  ): Promise<ToolFindResult> {
    try {
  ...
  public async executeTool(
    ...
    try {
      ...
      const response: ToolExecutionResponse =
        await this.components.toolExecutor.execute(enhancedRequest);
  ...
  public async executeBatch(
    ...
    context: IAgentExecutionContext,
  ): Promise<ToolBatchExecuteResult> {
    const executedAt = new Date();
    ...
    try {
      ...
      if (request.mode === BatchExecutionMode.PARALLEL) {
        ...
        if (
          ...
          request.maxConcurrency < promises.length
        ) {
      ...
      return {
        success:
          failedExecutions === 0 ||
          (!request.failFast && successfulExecutions > 0),
  ...
  public getMetrics(): ToolAgentMetrics {
    ...
    const averageLatencies: Record<ToolOperation, number> = {} as Record<
      ToolOperation,
      number
    >;
      ...
      toolUsage[toolName] = {
        ...
        averageLatency:
          stats.latencies.length > 0
            ? stats.latencies.reduce((sum, time) => sum + time, 0) /
  ...
  private validateAndParseRequest(input: unknown): ToolOperationRequest {
    ...
    if (
      !Object.values(ToolOperation).includes(request.operation as ToolOperation)
    ) {
      throw new InvalidToolOperationException(
        request.operation as string,
        "Unsupported operation",
    ...
    if (
      operation === ToolOperation.FIND &&
      (!request.toolName || typeof request.toolName !== "string")
    ) {
      throw new InvalidToolOperationException(
        operation,
        "Tool name is required for find operation",
      );
    }
    ...
    if (operation === ToolOperation.EXECUTE) {
      if (!request.toolName || typeof request.toolName !== "string") {
        throw new InvalidToolOperationException(
          operation,
          "Tool name is required for execute operation",
        );
      }
    ...
    if (operation === ToolOperation.EXECUTE_BATCH) {
      if (!request.executions || !Array.isArray(request.executions)) {
        throw new InvalidToolOperationException(
          operation,
          "Executions array is required for batch execute operation",
        );
      }
      if (
        !request.mode ||
        !Object.values(BatchExecutionMode).includes(
          request.mode as BatchExecutionMode,
        )
      ) {
        throw new InvalidToolOperationException(
          operation,
          "Valid execution mode is required for batch execute operation",
        );
      }
    }
    ...
    if (operation === ToolOperation.VALIDATE) {
      if (!request.toolName || typeof request.toolName !== "string") {
        throw new InvalidToolOperationException(
          operation,
          "Tool name is required for validate operation",
        );
      }
      if (request.input === undefined) {
        throw new InvalidToolOperationException(
          operation,
          "Input is required for validate operation",
        );
      }
    }
