...
export class PluginAgent implements IAgent {
  ...
  // Metrics
  private readonly operationCounts: Record<PluginOperation, number> =
    {} as Record<PluginOperation, number>;
  private readonly successCounts: Record<PluginOperation, number> =
    {} as Record<PluginOperation, number>;
  private readonly errorCounts: Record<PluginOperation, number> = {} as Record<
    PluginOperation,
    number
  >;
  private readonly latencies: Record<PluginOperation, number[]> = {} as Record<
    PluginOperation,
    number[]
  >;
  ...
  private pluginUsageStats: Record<
    string,
    { usageCount: number; lastUsed?: Date; averageLoadTime?: number }
  > = {};
  ...
  constructor(components: PluginAgentComponents) {
    this.metadata = {
      ...
      name: "Plugin Agent",
      description:
        "Built-in agent for plugin management operations using the existing PluginRegistry and PluginLoader infrastructure",
      version: "1.0.0",
    ...
    this.capabilities = [
      {
        ...
        description:
          "Discover plugins from specified search paths with filtering and validation",
      ...
      },
      {
        ...
        description:
          "Load, unload, and reload plugins with dependency resolution and validation",
        inputSchema: {
          operation: "string",
          pluginId: "string",
          loadOptions: "object",
      ...
      },
      {
        ...
        description:
          "Validate plugin integrity, dependencies, and compatibility",
      ...
      },
      {
        ...
        description:
          "List and manage registered plugins with status and metadata",
  ...
  public async getHealth(): Promise<AgentHealth> {
    try {
      ...
      if (
        ...
        !this.components.pluginRegistry ||
        !this.components.pluginLoader
      ) {
        status = "unhealthy";
    ...
    try {
      ...
      switch (request.operation) {
        case PluginOperation.DISCOVER:
          result = await this.discoverPlugins(
            request as PluginDiscoverRequest,
            context,
          );
          break;
        ...
        case PluginOperation.UNLOAD:
          result = await this.unloadPlugin(
            request as PluginUnloadRequest,
            context,
          );
          break;
        case PluginOperation.RELOAD:
          result = await this.reloadPlugin(
            request as PluginReloadRequest,
            context,
          );
          break;
        case PluginOperation.VALIDATE:
          result = await this.validatePlugin(
            request as PluginValidateRequest,
            context,
          );
          break;
        case PluginOperation.LIST:
          result = await this.listPlugins(
            request as PluginListRequest,
            context,
          );
          break;
        default:
  ...
  public async discoverPlugins(
    ...
    try {
      ...
      const discoveredPlugins = await this.components.pluginLoader.discover(
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Unknown plugin discovery error";
      throw new PluginDiscoveryAgentException(
        request.discoveryOptions.searchPaths,
        errorMsg,
      );
    }
  }
  ...
  public async loadPlugin(
    ...
    try {
      ...
      const pluginDescriptor = await this.components.pluginLoader.load(
        request.pluginId,
        request.loadOptions,
      );
      ...
      if (pluginDescriptor.loadTime) {
        const currentAvg =
          this.pluginUsageStats[request.pluginId].averageLoadTime || 0;
  ...
  public async unloadPlugin(
    ...
    try {
      ...
      const unloaded = await this.components.pluginLoader.unload(
        request.pluginId,
      );
      ...
      const errorMsg =
        error instanceof Error ? error.message : "Unknown plugin unload error";
  ...
  public async reloadPlugin(
    ...
    try {
      ...
      const pluginDescriptor = await this.components.pluginLoader.reload(
        request.pluginId,
        request.loadOptions,
      );
      ...
      const errorMsg =
        error instanceof Error ? error.message : "Unknown plugin reload error";
  ...
  public async validatePlugin(
    ...
    try {
      ...
      const valid = await this.components.pluginLoader.validate(
        request.pluginId,
      );

      // Get detailed validation result from registry
      const validationResult = await this.components.pluginRegistry.validate(
        request.pluginId,
      );
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Unknown plugin validation error";
  ...
  public async listPlugins(
    ...
    try {
      ...
      const errorMsg =
        error instanceof Error ? error.message : "Unknown plugin listing error";
    ...
    try {
      ...
      try {
        ...
        loadedPlugins = plugins.filter(
          (p) => p.status === "loaded" || p.status === "active",
        ).length;
        ...
        metadata: {
          ...
          discoverySuccessRate:
            this.discoveryAttempts > 0
              ? this.totalDiscoveries / this.discoveryAttempts
              : 0,
  ...
  public getMetrics(): PluginAgentMetrics {
    ...
    const averageLatencies: Record<PluginOperation, number> = {} as Record<
      PluginOperation,
      number
    >;
    ...
    return {
      ...
      operationStats: {
        discoverySuccessRate: this.calculateOperationSuccessRate(
          PluginOperation.DISCOVER,
        ),
        loadSuccessRate: this.calculateOperationSuccessRate(
          PluginOperation.LOAD,
        ),
        unloadSuccessRate: this.calculateOperationSuccessRate(
          PluginOperation.UNLOAD,
        ),
        reloadSuccessRate: this.calculateOperationSuccessRate(
          PluginOperation.RELOAD,
        ),
        validationSuccessRate: this.calculateOperationSuccessRate(
          PluginOperation.VALIDATE,
        ),
        averagePluginsPerDiscovery:
          this.totalDiscoveries > 0
            ? this.totalPluginsDiscovered / this.totalDiscoveries
            : 0,
        pluginUsageStats: { ...this.pluginUsageStats },
      },
    };
  ...
  private validateAndParseRequest(input: unknown): PluginOperationRequest {
    ...
    if (
      !Object.values(PluginOperation).includes(
        request.operation as PluginOperation,
      )
    ) {
      throw new InvalidPluginOperationException(
        request.operation as string,
    ...
    if (operation === PluginOperation.DISCOVER) {
      if (
        !request.discoveryOptions ||
        typeof request.discoveryOptions !== "object"
      ) {
        throw new InvalidPluginOperationException(
          operation,
          "Discovery options are required for discover operation",
        );
      }
    }
    ...
    if (
      ...
      if (!request.pluginId || typeof request.pluginId !== "string") {
        throw new InvalidPluginOperationException(
          operation,
          "Plugin ID is required for plugin-specific operations",
        );
      }
    }
  ...
  private validateDiscoveryOptions(options: any): void {
    if (
      ...
      options.searchPaths.length === 0
    ) {
      throw new InvalidPluginDiscoveryOptionsException(["searchPaths"]);
    }

    // Validate each search path is a string
    const invalidPaths = options.searchPaths.filter(
      (path: any) => typeof path !== "string",
    );
    if (invalidPaths.length > 0) {
      throw new InvalidPluginDiscoveryOptionsException([
        "searchPaths (must be strings)",
      ]);
    }
    ...
    if (
      ...
      !Array.isArray(options.includePatterns)
    ) {
      throw new InvalidPluginDiscoveryOptionsException([
        "includePatterns (must be array if provided)",
      ]);
    }
    ...
    if (
      ...
      throw new InvalidPluginDiscoveryOptionsException([
        "excludePatterns (must be array if provided)",
      ]);
    }
  }
