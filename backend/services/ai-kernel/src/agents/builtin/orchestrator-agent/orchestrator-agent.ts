...
export class OrchestratorAgent implements IAgent {
  ...
  // Metrics
  private readonly operationCounts: Record<OrchestratorOperation, number> =
    {} as Record<OrchestratorOperation, number>;
  private readonly successCounts: Record<OrchestratorOperation, number> =
    {} as Record<OrchestratorOperation, number>;
  private readonly errorCounts: Record<OrchestratorOperation, number> =
    {} as Record<OrchestratorOperation, number>;
  private readonly latencies: Record<OrchestratorOperation, number[]> =
    {} as Record<OrchestratorOperation, number[]>;
  ...
  constructor(components: OrchestratorAgentComponents) {
    this.metadata = {
      ...
      description:
        "Built-in agent for orchestrating agent execution operations using the existing AgentOrchestrator infrastructure",
      version: "1.0.0",
    ...
    this.capabilities = [
      {
        ...
        description:
          "Orchestrate individual agent execution with lifecycle and context management",
      ...
      },
      {
        ...
        description:
          "Orchestrate complex workflow execution with step coordination and state management",
      ...
      },
      {
        ...
        description:
          "Orchestrate plan execution with task scheduling and dependency management",
      ...
      },
      {
        ...
        description:
          "Control and monitor orchestrated executions with cancellation and health monitoring",
    ...
    try {
      ...
      switch (request.operation) {
        case OrchestratorOperation.EXECUTE:
          result = await this.executeAgent(
            request as OrchestratorExecuteAgentRequest,
            context,
          );
          break;
        case OrchestratorOperation.EXECUTE_WORKFLOW:
          result = await this.executeWorkflow(
            request as OrchestratorExecuteWorkflowRequest,
            context,
          );
          break;
        case OrchestratorOperation.EXECUTE_PLAN:
          result = await this.executePlan(
            request as OrchestratorExecutePlanRequest,
            context,
          );
          break;
        case OrchestratorOperation.CANCEL:
          result = await this.cancelExecution(
            request as OrchestratorCancelExecutionRequest,
            context,
          );
          break;
        case OrchestratorOperation.HEALTH:
          result = await this.getOrchestratorHealth(
            request as GetHealthRequest,
            context,
          );
          break;
        default:
          throw new InvalidOrchestratorOperationException(
      ...
      if (input && typeof input === "object" && "operation" in input) {
        this.recordOperationError(
          input.operation as OrchestratorOperation,
          duration,
        );
      }
  ...
  public async executeAgent(
    request: OrchestratorExecuteAgentRequest,
    context: IAgentExecutionContext,
  ): Promise<OrchestratorExecuteAgentResult> {
    ...
    try {
      ...
      const orchestratorExecution =
        await this.components.agentOrchestrator.execute(request.agentRequest);
      ...
      throw new AgentExecutionAgentException(
        request.agentRequest.agentId,
        errorMsg,
      );
    }
  }
  ...
  public async executeWorkflow(
    ...
    context: IAgentExecutionContext,
  ): Promise<OrchestratorExecuteWorkflowResult> {
    const startedAt = new Date();
    ...
    try {
      ...
      // Execute using existing orchestrator
      const orchestratorExecution =
        await this.components.agentOrchestrator.executeWorkflow(
      ...
      throw new WorkflowExecutionAgentException(
        request.workflowRequest.workflowId,
        errorMsg,
      );
    }
  }
  ...
  public async executePlan(
    request: OrchestratorExecutePlanRequest,
    context: IAgentExecutionContext,
  ): Promise<OrchestratorExecutePlanResult> {
    const startedAt = new Date();
    ...
    try {
      ...
      // Execute using existing orchestrator
      const orchestratorExecution =
        await this.components.agentOrchestrator.executePlan(
          request.planRequest,
        );
  ...
  public async cancelExecution(
    request: OrchestratorCancelExecutionRequest,
    context: IAgentExecutionContext,
  ): Promise<OrchestratorCancelExecutionResult> {
    const cancelledAt = new Date();

    try {
      // Cancel using existing orchestrator
      const cancelled = await this.components.agentOrchestrator.cancel(
        request.executionId,
      );
      ...
      throw new ExecutionCancellationAgentException(
        request.executionId,
        errorMsg,
      );
    }
  ...
  public async getOrchestratorHealth(
    ...
    try {
      ...
      const orchestratorHealth =
        await this.components.agentOrchestrator.health();
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Unknown orchestrator health error";
  ...
  public async getOrchestratorAgentHealth(): Promise<OrchestratorAgentHealth> {
    try {
      ...
      return {
        ...
        totalExecutions:
          orchestratorHealth?.totalExecutions || this.totalExecutions,
        errors: [...this.errors],
        warnings: [...this.warnings],
  ...
  public getMetrics(): OrchestratorAgentMetrics {
    ...
    const averageLatencies: Record<OrchestratorOperation, number> =
      {} as Record<OrchestratorOperation, number>;
    ...
    return {
      ...
      executionStats: {
        ...
        averageExecutionTime:
          (averageLatencies[OrchestratorOperation.EXECUTE] +
            averageLatencies[OrchestratorOperation.EXECUTE_WORKFLOW] +
            averageLatencies[OrchestratorOperation.EXECUTE_PLAN]) /
          3,
      },
      orchestrationStats: {
        totalCancellations: this.totalCancellations,
  ...
  private validateAndParseRequest(
    ...
    if (
      !Object.values(OrchestratorOperation).includes(
        request.operation as OrchestratorOperation,
      )
    ) {
      throw new InvalidOrchestratorOperationException(
        request.operation as string,
    ...
    if (operation === OrchestratorOperation.EXECUTE) {
      if (!request.agentRequest || typeof request.agentRequest !== "object") {
        throw new InvalidOrchestratorOperationException(
          operation,
          "Agent request is required for execute operation",
        );
      }
    }
    ...
    if (operation === OrchestratorOperation.EXECUTE_WORKFLOW) {
      if (
        !request.workflowRequest ||
        typeof request.workflowRequest !== "object"
      ) {
        throw new InvalidOrchestratorOperationException(
          operation,
          "Workflow request is required for execute-workflow operation",
        );
      }
    }
    ...
    if (operation === OrchestratorOperation.EXECUTE_PLAN) {
      if (!request.planRequest || typeof request.planRequest !== "object") {
        throw new InvalidOrchestratorOperationException(
          operation,
          "Plan request is required for execute-plan operation",
        );
      }
    }
    ...
    if (operation === OrchestratorOperation.CANCEL) {
      if (!request.executionId || typeof request.executionId !== "string") {
        throw new InvalidOrchestratorOperationException(
          operation,
          "Execution ID is required for cancel operation",
        );
      }
    }
  ...
  private validateAgentExecutionRequest(request: any): void {
    ...
    const missingFields = requiredFields.filter(
      (field) => !(field in request) || request[field] === undefined,
    );
