...
import { IWorkflowEngine, WorkflowValidationResult } from "../../workflow/workflow.interface";
...
export class WorkflowAgent implements IAgent {
  ...
  // Metrics
  private readonly operationCounts: Record<WorkflowOperation, number> = {} as Record<WorkflowOperation, number>;
  private readonly successCounts: Record<WorkflowOperation, number> = {} as Record<WorkflowOperation, number>;
  private readonly errorCounts: Record<WorkflowOperation, number> = {} as Record<WorkflowOperation, number>;
  private readonly latencies: Record<WorkflowOperation, number[]> = {} as Record<WorkflowOperation, number[]>;
  ...
  constructor(components: WorkflowAgentComponents) {
    this.metadata = {
      id: "workflow-agent",
      name: "Workflow Agent",
      description: "Built-in agent for workflow operations using the existing WorkflowEngine infrastructure",
      version: "1.0.0",
    ...
    this.capabilities = [
      {
        ...
        description: "Execute workflows with step coordination and state management",
        inputSchema: { operation: "string", workflowId: "string", input: "object", context: "object" },
      ...
      },
      {
        ...
        description: "Validate workflow definitions for correctness and completeness",
      ...
      },
      {
        ...
        description: "Control workflow execution with pause, resume, and cancel operations",
    ...
    try {
      ...
      switch (request.operation) {
        case WorkflowOperation.EXECUTE_WORKFLOW:
          result = await this.executeWorkflow(request as WorkflowExecuteRequest, context);
          break;
        case WorkflowOperation.VALIDATE_WORKFLOW:
          result = await this.validateWorkflow(request as WorkflowValidateRequest, context);
          break;
        case WorkflowOperation.PAUSE_WORKFLOW:
          result = await this.pauseWorkflow(request as WorkflowPauseRequest, context);
          break;
        case WorkflowOperation.RESUME_WORKFLOW:
          result = await this.resumeWorkflow(request as WorkflowResumeRequest, context);
          break;
        case WorkflowOperation.CANCEL_WORKFLOW:
          result = await this.cancelWorkflow(request as WorkflowCancelRequest, context);
          break;
        default:
          throw new InvalidWorkflowOperationException(request.operation as string, "Unsupported operation");
      }
      ...
      if (input && typeof input === 'object' && 'operation' in input) {
        this.recordOperationError(input.operation as WorkflowOperation, duration);
      }
  ...
  public async executeWorkflow(request: WorkflowExecuteRequest, context: IAgentExecutionContext): Promise<WorkflowExecuteResult> {
  ...
  public async validateWorkflow(request: WorkflowValidateRequest, context: IAgentExecutionContext): Promise<WorkflowValidateResult> {
    const validatedAt = new Date();
    
    try {
      // Use existing workflow engine to validate
      const validationResult: WorkflowValidationResult = await this.components.workflowEngine.validate(request.workflow);
      ...
      const errorMsg = error instanceof Error ? error.message : "Unknown workflow validation error";
      throw new WorkflowValidationAgentException(request.workflow.workflowId, [errorMsg]);
    }
  }

  public async pauseWorkflow(request: WorkflowPauseRequest, context: IAgentExecutionContext): Promise<WorkflowPauseResult> {
    const pausedAt = new Date();
    
    try {
      // Use existing workflow engine to pause
      const paused = await this.components.workflowEngine.pause(request.executionId);
      ...
      const errorMsg = error instanceof Error ? error.message : "Unknown workflow pause error";
      throw new WorkflowControlException(request.executionId, "pause", errorMsg);
    }
  }

  public async resumeWorkflow(request: WorkflowResumeRequest, context: IAgentExecutionContext): Promise<WorkflowResumeResult> {
    const resumedAt = new Date();
    
    try {
      // Use existing workflow engine to resume
      const resumed = await this.components.workflowEngine.resume(request.executionId);
      ...
      const errorMsg = error instanceof Error ? error.message : "Unknown workflow resume error";
      throw new WorkflowControlException(request.executionId, "resume", errorMsg);
    }
  }

  public async cancelWorkflow(request: WorkflowCancelRequest, context: IAgentExecutionContext): Promise<WorkflowCancelResult> {
    const cancelledAt = new Date();
    
    try {
      // Use existing workflow engine to cancel
      const cancelled = await this.components.workflowEngine.cancel(request.executionId);
      ...
      const errorMsg = error instanceof Error ? error.message : "Unknown workflow cancel error";
      throw new WorkflowControlException(request.executionId, "cancel", errorMsg);
    }
  }
  ...
  public async getWorkflowAgentHealth(): Promise<WorkflowAgentHealth> {
    try {
      ...
      return {
        ...
        completedExecutions: this.successCounts[WorkflowOperation.EXECUTE_WORKFLOW] || 0,
        failedExecutions: this.errorCounts[WorkflowOperation.EXECUTE_WORKFLOW] || 0,
        errors: [...this.errors],
        warnings: [...this.warnings],
        lastActivity: this.lastActivity,
        metadata: {
          totalValidations: this.totalValidations,
          validationSuccessRate: this.totalValidations > 0 ? this.validWorkflows / this.totalValidations : 0,
  ...
  public getMetrics(): WorkflowAgentMetrics {
    ...
    const averageLatencies: Record<WorkflowOperation, number> = {} as Record<WorkflowOperation, number>;
    ...
    const successfulExecutions = this.successCounts[WorkflowOperation.EXECUTE_WORKFLOW] || 0;
    const failedExecutions = this.errorCounts[WorkflowOperation.EXECUTE_WORKFLOW] || 0;
    ...
    return {
      ...
      workflowStats: {
        ...
        averageExecutionTime: averageLatencies[WorkflowOperation.EXECUTE_WORKFLOW] || 0,
  ...
  private validateAndParseRequest(input: unknown): WorkflowOperationRequest {
    if (!input || typeof input !== 'object') {
      throw new InvalidWorkflowOperationException("unknown", "Input must be an object");
    }
    
    const request = input as Record<string, unknown>;
    
    if (!request.operation || typeof request.operation !== 'string') {
      throw new InvalidWorkflowOperationException("unknown", "Operation is required and must be a string");
    }
    
    if (!Object.values(WorkflowOperation).includes(request.operation as WorkflowOperation)) {
      throw new InvalidWorkflowOperationException(request.operation as string, "Unsupported operation");
    }
    ...
    if (operation === WorkflowOperation.EXECUTE_WORKFLOW) {
      if (!request.workflowId || typeof request.workflowId !== 'string') {
        throw new InvalidWorkflowOperationException(operation, "Workflow ID is required for execute operation");
      }
      if (request.input === undefined) {
        throw new InvalidWorkflowOperationException(operation, "Input is required for execute operation");
      }
      if (!request.context || typeof request.context !== 'object') {
        throw new InvalidWorkflowOperationException(operation, "Context is required for execute operation");
      }
    }
    
    if (operation === WorkflowOperation.VALIDATE_WORKFLOW) {
      if (!request.workflow || typeof request.workflow !== 'object') {
        throw new InvalidWorkflowOperationException(operation, "Workflow is required for validate operation");
      }
    }
    
    if ([WorkflowOperation.PAUSE_WORKFLOW, WorkflowOperation.RESUME_WORKFLOW, WorkflowOperation.CANCEL_WORKFLOW].includes(operation)) {
      if (!request.executionId || typeof request.executionId !== 'string') {
        throw new InvalidWorkflowOperationException(operation, "Execution ID is required for control operations");
      }
    }
  ...
  private recordOperationSuccess(operation: WorkflowOperation, duration: number): void {
  ...
  private recordOperationError(operation: WorkflowOperation, duration: number): void {
