export class WorkflowAgentException extends Error {
  ...
  constructor(operation?: string, message?: string) {
    super(message || `Workflow agent operation${operation ? ` '${operation}'` : ''} failed`);
...
export class WorkflowValidationAgentException extends Error {
  ...
  constructor(workflowId: string, errors: string[], message?: string) {
    super(message || `Workflow validation failed for '${workflowId}': ${errors.join(", ")}`);
    this.workflowId = workflowId;
...
export class WorkflowControlException extends Error {
  ...
  constructor(executionId: string, operation: string, reason: string, message?: string) {
    super(message || `Workflow ${operation} failed for execution '${executionId}': ${reason}`);
...
export class InvalidWorkflowContextException extends Error {
  ...
  constructor(missingFields: string[], message?: string) {
    super(message || `Invalid workflow context - missing fields: ${missingFields.join(', ')}`);
