...
import { MemoryAgent, MemoryAgentComponents } from "../memory-agent";
import { ToolAgent, ToolAgentComponents } from "../tool-agent";
import { PlannerAgent, PlannerAgentComponents } from "../planner-agent";
import { WorkflowAgent, WorkflowAgentComponents } from "../workflow-agent";
import { ExecutionAgent, ExecutionAgentComponents } from "../execution-agent";
import {
  CoordinatorAgent,
  CoordinatorAgentComponents,
} from "../coordinator-agent";
import { PluginAgent, PluginAgentComponents } from "../plugin-agent";
import {
  CommunicationAgent,
  CommunicationAgentComponents,
} from "../communication-agent";
import { SchedulerAgent, SchedulerAgentComponents } from "../scheduler-agent";
import {
  OrchestratorAgent,
...
export class MissingComponentsException extends Error {
  ...
  constructor(agentType: string, missingComponent: string, message?: string) {
    super(
      message ||
        `Missing required components for '${agentType}': ${missingComponent}`,
    );
    this.agentType = agentType;
...
export class BuiltinAgentFactory {
  ...
  public createAgent(agentType: BuiltinAgentType): IAgent {
    try {
      ...
      if (
        error instanceof UnsupportedAgentTypeException ||
        error instanceof MissingComponentsException
      ) {
        throw error;
      }
  ...
  public isAgentTypeSupported(agentType: string): boolean {
    return Object.values(BuiltinAgentType).includes(
      agentType as BuiltinAgentType,
    );
  }
  ...
  private createCommunicationAgent(): CommunicationAgent {
    if (!this.components.communicationComponents) {
      throw new MissingComponentsException(
        BuiltinAgentType.COMMUNICATION,
