...
import { IAgentPlanner, AgentPlanner, PlanningRequest } from "../../planner/agent-planner";
...
export class PlannerAgent implements IAgent {
  ...
  // Metrics
  private readonly operationCounts: Record<PlannerOperation, number> = {} as Record<PlannerOperation, number>;
  private readonly successCounts: Record<PlannerOperation, number> = {} as Record<PlannerOperation, number>;
  private readonly errorCounts: Record<PlannerOperation, number> = {} as Record<PlannerOperation, number>;
  private readonly latencies: Record<PlannerOperation, number[]> = {} as Record<PlannerOperation, number[]>;
  ...
  constructor(components: PlannerAgentComponents) {
    this.metadata = {
      id: "planner-agent",
      name: "Planner Agent",
      description: "Built-in agent for planning operations using the existing Agent Planner infrastructure",
      version: "1.0.0",
    ...
    this.capabilities = [
      ...
      },
      {
        ...
        description: "Estimate execution time and resource requirements for plans",
        inputSchema: { operation: "string", plan: "object" },
      ...
      },
      {
        ...
        inputSchema: { operation: "string", existingPlan: "object", modifications: "object" },
        outputSchema: { success: "boolean", plan: "object" },
        parameters: {},
        dependencies: []
      }
    ...
    try {
      ...
      switch (request.operation) {
        case PlannerOperation.PLAN:
          result = await this.createPlan(request as PlannerPlanRequest, context);
          break;
        ...
        case PlannerOperation.VALIDATE:
          result = await this.validatePlan(request as PlannerValidateRequest, context);
          break;
        case PlannerOperation.ESTIMATE:
          result = await this.estimatePlan(request as PlannerEstimateRequest, context);
          break;
        default:
          throw new InvalidPlannerOperationException(request.operation as string, "Unsupported operation");
      }
      ...
      if (input && typeof input === 'object' && 'operation' in input) {
        this.recordOperationError(input.operation as PlannerOperation, duration);
      }
  ...
  public async createPlan(request: PlannerPlanRequest, context: IAgentExecutionContext): Promise<PlannerPlanResult> {
    ...
    try {
      ...
      const errorMsg = error instanceof Error ? error.message : "Unknown planning error";
      throw new PlanningException(request.agentId, errorMsg);
    }
  }

  public async replan(request: PlannerReplanRequest, context: IAgentExecutionContext): Promise<PlannerReplanResult> {
    ...
    try {
      ...
      if (request.modifications) {
        ...
        // Remove tasks
        if (mods.removeTasks) {
          modifiedTasks = modifiedTasks.filter(task => !mods.removeTasks!.includes(task.taskId));
        ...
        if (mods.updateTasks) {
          for (const update of mods.updateTasks) {
            const taskIndex = modifiedTasks.findIndex(task => task.taskId === update.taskId);
            if (taskIndex >= 0) {
              modifiedTasks[taskIndex] = { ...modifiedTasks[taskIndex], ...update.updates };
      ...
      const newPlanRequest: PlanningRequest = {
        ...
        priority: request.modifications?.changePriority || originalPlan.priority,
        tasks: modifiedTasks,
        executionMode: request.modifications?.changeMode || originalPlan.executionMode,
      ...
      const newEstimatedDuration = await this.components.planner.estimate(newPlan);
      ...
      const errorMsg = error instanceof Error ? error.message : "Unknown replanning error";
      throw new ReplanningException(request.existingPlan.planId, errorMsg);
    }
  }

  public async validatePlan(request: PlannerValidateRequest, context: IAgentExecutionContext): Promise<PlannerValidateResult> {
    const validatedAt = new Date();
    
    try {
      // Use existing planner to validate plan
      const validationResult = await this.components.planner.validatePlan(request.plan);
  ...
  public async estimatePlan(request: PlannerEstimateRequest, context: IAgentExecutionContext): Promise<PlannerEstimateResult> {
    const estimatedAt = new Date();
    
    try {
      // Use existing planner to estimate plan
      const estimatedDurationMs = await this.components.planner.estimate(request.plan);
      ...
      if (request.includeTaskBreakdown) {
        taskBreakdown = request.plan.tasks.map(task => ({
          ...
          dependencies: task.dependencies.map(dep => typeof dep === 'string' ? dep : dep.taskId)
        }));
      }
      ...
      const errorMsg = error instanceof Error ? error.message : "Unknown estimation error";
  ...
  public async getPlannerAgentHealth(): Promise<PlannerAgentHealth> {
    try {
      ...
      const averagePlanComplexity = totalPlans > 0 
        ? Array.from(this.activePlans.values()).reduce((sum, plan) => sum + plan.tasks.length, 0) / this.activePlans.size || 0
        : 0;
      ...
      return {
        ...
        metadata: {
          ...
          validationSuccessRate: this.totalValidations > 0 ? this.validPlans / this.totalValidations : 0
        }
      };
  ...
  public getMetrics(): PlannerAgentMetrics {
    ...
    const averageLatencies: Record<PlannerOperation, number> = {} as Record<PlannerOperation, number>;
    ...
    const averageTasksPerPlan = activePlansArray.length > 0 
      ? activePlansArray.reduce((sum, plan) => sum + plan.tasks.length, 0) / activePlansArray.length 
      : 0;
  ...
  private validateAndParseRequest(input: unknown): PlannerOperationRequest {
    if (!input || typeof input !== 'object') {
      throw new InvalidPlannerOperationException("unknown", "Input must be an object");
    }
    
    const request = input as Record<string, unknown>;
    
    if (!request.operation || typeof request.operation !== 'string') {
      throw new InvalidPlannerOperationException("unknown", "Operation is required and must be a string");
    }
    
    if (!Object.values(PlannerOperation).includes(request.operation as PlannerOperation)) {
      throw new InvalidPlannerOperationException(request.operation as string, "Unsupported operation");
    }
    ...
    if (operation === PlannerOperation.PLAN) {
      if (!request.agentId || typeof request.agentId !== 'string') {
        throw new InvalidPlannerOperationException(operation, "Agent ID is required for plan operation");
      }
      if (!request.workspaceId || typeof request.workspaceId !== 'string') {
        throw new InvalidPlannerOperationException(operation, "Workspace ID is required for plan operation");
      }
      if (!request.name || typeof request.name !== 'string') {
        throw new InvalidPlannerOperationException(operation, "Name is required for plan operation");
      }
      if (!request.tasks || !Array.isArray(request.tasks)) {
        throw new InvalidPlannerOperationException(operation, "Tasks array is required for plan operation");
      }
    }
    
    if (operation === PlannerOperation.REPLAN) {
      if (!request.existingPlan || typeof request.existingPlan !== 'object') {
        throw new InvalidPlannerOperationException(operation, "Existing plan is required for replan operation");
      }
    }
    
    if (operation === PlannerOperation.VALIDATE || operation === PlannerOperation.ESTIMATE) {
      if (!request.plan || typeof request.plan !== 'object') {
        throw new InvalidPlannerOperationException(operation, "Plan is required for validate/estimate operations");
      }
    }
  ...
  private recordOperationSuccess(operation: PlannerOperation, duration: number): void {
  ...
  private recordOperationError(operation: PlannerOperation, duration: number): void {
