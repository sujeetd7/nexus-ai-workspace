...
export class SchedulerAgent implements IAgent {
  ...
  // Metrics
  private readonly operationCounts: Record<SchedulerOperation, number> = {} as Record<SchedulerOperation, number>;
  private readonly successCounts: Record<SchedulerOperation, number> = {} as Record<SchedulerOperation, number>;
  private readonly errorCounts: Record<SchedulerOperation, number> = {} as Record<SchedulerOperation, number>;
  private readonly latencies: Record<SchedulerOperation, number[]> = {} as Record<SchedulerOperation, number[]>;
  ...
  constructor(components: SchedulerAgentComponents) {
    this.metadata = {
      ...
      description: "Built-in agent for task scheduling operations using the existing AgentScheduler infrastructure",
      version: "1.0.0",
    ...
    this.capabilities = [
      {
        ...
        description: "Schedule individual tasks with priority and dependency management",
      ...
      },
      {
        ...
        description: "Enqueue multiple tasks for batch processing with efficient queue management",
      ...
      },
      {
        ...
        description: "Dequeue, cancel, and control task execution with state management",
      ...
      },
      {
        ...
        description: "Control scheduler operation with pause, resume, and metrics capabilities",
    ...
    try {
      ...
      switch (request.operation) {
        case SchedulerOperation.SCHEDULE:
          result = await this.scheduleTask(request as ScheduleTaskRequest, context);
          break;
        case SchedulerOperation.ENQUEUE:
          result = await this.enqueueTasks(request as EnqueueTasksRequest, context);
          break;
        case SchedulerOperation.DEQUEUE:
          result = await this.dequeueTask(request as DequeueTaskRequest, context);
          break;
        ...
        case SchedulerOperation.PAUSE:
          result = await this.pauseScheduler(request as PauseSchedulerRequest, context);
          break;
        case SchedulerOperation.RESUME:
          result = await this.resumeScheduler(request as ResumeSchedulerRequest, context);
          break;
        case SchedulerOperation.METRICS:
          result = await this.getSchedulerMetrics(request as GetMetricsRequest, context);
          break;
        default:
          throw new InvalidSchedulerOperationException(request.operation as string, "Unsupported operation");
      }
      ...
      if (input && typeof input === 'object' && 'operation' in input) {
        this.recordOperationError(input.operation as SchedulerOperation, duration);
      }
  ...
  public async scheduleTask(request: ScheduleTaskRequest, context: IAgentExecutionContext): Promise<ScheduleTaskResult> {
    ...
    try {
      ...
      const errorMsg = error instanceof Error ? error.message : "Unknown task scheduling error";
      throw new TaskSchedulingException(request.task.taskId, errorMsg);
    }
  }

  public async enqueueTasks(request: EnqueueTasksRequest, context: IAgentExecutionContext): Promise<EnqueueTasksResult> {
    ...
    try {
      ...
      const errorMsg = error instanceof Error ? error.message : "Unknown task enqueue error";
      throw new TaskEnqueueException(request.tasks.length, errorMsg);
    }
  }

  public async dequeueTask(request: DequeueTaskRequest, context: IAgentExecutionContext): Promise<DequeueTaskResult> {
    ...
    try {
      ...
      const errorMsg = error instanceof Error ? error.message : "Unknown task dequeue error";
      throw new TaskDequeueException(errorMsg);
    }
  }

  public async cancelTask(request: CancelTaskRequest, context: IAgentExecutionContext): Promise<CancelTaskResult> {
    const cancelledAt = new Date();
    
    try {
      // Cancel using existing scheduler
      const cancelled = await this.components.agentScheduler.cancel(request.taskId);
      ...
      const errorMsg = error instanceof Error ? error.message : "Unknown task cancellation error";
      throw new TaskCancellationException(request.taskId, errorMsg);
    }
  }

  public async pauseScheduler(request: PauseSchedulerRequest, context: IAgentExecutionContext): Promise<PauseSchedulerResult> {
    ...
    try {
      ...
      const errorMsg = error instanceof Error ? error.message : "Unknown scheduler pause error";
      throw new SchedulerControlException("pause", errorMsg);
    }
  }

  public async resumeScheduler(request: ResumeSchedulerRequest, context: IAgentExecutionContext): Promise<ResumeSchedulerResult> {
    ...
    try {
      ...
      const errorMsg = error instanceof Error ? error.message : "Unknown scheduler resume error";
      throw new SchedulerControlException("resume", errorMsg);
    }
  }

  public async getSchedulerMetrics(request: GetMetricsRequest, context: IAgentExecutionContext): Promise<GetMetricsResult> {
    ...
    try {
      ...
      const errorMsg = error instanceof Error ? error.message : "Unknown scheduler metrics error";
  ...
  public async getSchedulerAgentHealth(): Promise<SchedulerAgentHealth> {
    try {
      ...
      try {
        ...
        queueSizes = { priority: 0, execution: 0, waiting: 0, completed: 0, failed: 0 };
      }
      ...
      return {
        ...
        metadata: {
          ...
          averageTasksPerEnqueue: this.totalEnqueued > 0 ? this.totalTasksInEnqueue / this.totalEnqueued : 0,
  ...
  public getMetrics(): SchedulerAgentMetrics {
    ...
    const averageLatencies: Record<SchedulerOperation, number> = {} as Record<SchedulerOperation, number>;
    ...
    const queueSizes = { priority: 0, execution: 0, waiting: 0, completed: 0, failed: 0 };
    ...
    return {
      ...
      schedulingStats: {
        ...
        totalResumes: this.totalResumes,
        averageTasksPerEnqueue: this.totalEnqueued > 0 ? this.totalTasksInEnqueue / this.totalEnqueued : 0,
        schedulingSuccessRate: this.calculateOperationSuccessRate(SchedulerOperation.SCHEDULE),
        cancellationSuccessRate: this.calculateOperationSuccessRate(SchedulerOperation.CANCEL),
        dequeuingSuccessRate: this.calculateOperationSuccessRate(SchedulerOperation.DEQUEUE)
      },
      taskStats: {
  ...
  private validateAndParseRequest(input: unknown): SchedulerOperationRequest {
    if (!input || typeof input !== 'object') {
      throw new InvalidSchedulerOperationException("unknown", "Input must be an object");
    }
    
    const request = input as Record<string, unknown>;
    
    if (!request.operation || typeof request.operation !== 'string') {
      throw new InvalidSchedulerOperationException("unknown", "Operation is required and must be a string");
    }
    
    if (!Object.values(SchedulerOperation).includes(request.operation as SchedulerOperation)) {
      throw new InvalidSchedulerOperationException(request.operation as string, "Unsupported operation");
    }
    ...
    if (operation === SchedulerOperation.SCHEDULE) {
      if (!request.task || typeof request.task !== 'object') {
        throw new InvalidSchedulerOperationException(operation, "Task is required for schedule operation");
      }
    }
    
    if (operation === SchedulerOperation.ENQUEUE) {
      if (!Array.isArray(request.tasks)) {
        throw new InvalidSchedulerOperationException(operation, "Tasks array is required for enqueue operation");
      }
    }
    
    if (operation === SchedulerOperation.CANCEL) {
      if (!request.taskId || typeof request.taskId !== 'string') {
        throw new InvalidSchedulerOperationException(operation, "Task ID is required for cancel operation");
      }
    }
  ...
  private validateTask(task: AgentTask): void {
    ...
    const missingFields = requiredFields.filter(field => !(field in task) || task[field as keyof AgentTask] === undefined);
  ...
  private validateTasksArray(tasks: AgentTask[]): void {
    ...
    if (uniqueIds.size !== taskIds.length) {
      throw new InvalidTasksArrayException("Duplicate task IDs found in tasks array");
    }
  }
  ...
  private recordOperationSuccess(operation: SchedulerOperation, duration: number): void {
  ...
  private recordOperationError(operation: SchedulerOperation, duration: number): void {
