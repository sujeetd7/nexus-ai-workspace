...
import { AgentMessage, MessageType, MessageStatus, AgentMessageBuilder } from "../../communication/agent-message";
...
export class CommunicationAgent implements IAgent {
  ...
  // Metrics
  private readonly operationCounts: Record<CommunicationOperation, number> = {} as Record<CommunicationOperation, number>;
  private readonly successCounts: Record<CommunicationOperation, number> = {} as Record<CommunicationOperation, number>;
  private readonly errorCounts: Record<CommunicationOperation, number> = {} as Record<CommunicationOperation, number>;
  private readonly latencies: Record<CommunicationOperation, number[]> = {} as Record<CommunicationOperation, number[]>;
  ...
  private messagesByType: Record<MessageType, number> = {} as Record<MessageType, number>;
  private messagesByStatus: Record<MessageStatus, number> = {} as Record<MessageStatus, number>;
  private messagesByPriority: Record<AgentPriority, number> = {} as Record<AgentPriority, number>;
  private channelUsage: Record<string, { subscriptionCount: number; messageCount: number; lastActivity?: Date; }> = {};

  constructor(components: CommunicationAgentComponents) {
    this.metadata = {
      id: "communication-agent",
      name: "Communication Agent",
      description: "Built-in agent for inter-agent communication operations using the existing CommunicationManager infrastructure",
      version: "1.0.0",
    ...
    this.capabilities = [
      {
        ...
        description: "Send and receive direct messages between agents with delivery confirmation",
      ...
      },
      {
        ...
        description: "Broadcast messages to multiple agents with recipient tracking",
        inputSchema: { operation: "string", message: "object" },
      ...
      },
      {
        ...
        description: "Subscribe and unsubscribe agents from communication channels",
        inputSchema: { operation: "string", agentId: "string", channelId: "string" },
      ...
      },
      {
        ...
        description: "Receive messages from agent inboxes with filtering and timeout support",
        inputSchema: { operation: "string", agentId: "string", timeout: "number" },
    ...
    try {
      ...
      switch (request.operation) {
        case CommunicationOperation.SEND:
          result = await this.sendMessage(request as SendMessageRequest, context);
          break;
        case CommunicationOperation.RECEIVE:
          result = await this.receiveMessage(request as ReceiveMessageRequest, context);
          break;
        case CommunicationOperation.BROADCAST:
          result = await this.broadcastMessage(request as BroadcastMessageRequest, context);
          break;
        case CommunicationOperation.SUBSCRIBE:
          result = await this.subscribeToChannel(request as SubscribeChannelRequest, context);
          break;
        case CommunicationOperation.UNSUBSCRIBE:
          result = await this.unsubscribeFromChannel(request as UnsubscribeChannelRequest, context);
          break;
        default:
          throw new InvalidCommunicationOperationException(request.operation as string, "Unsupported operation");
      }
      ...
      if (input && typeof input === 'object' && 'operation' in input) {
        this.recordOperationError(input.operation as CommunicationOperation, duration);
      }
  ...
  public async sendMessage(request: SendMessageRequest, context: IAgentExecutionContext): Promise<SendMessageResult> {
    ...
    try {
      ...
      this.failedMessages++;
      const errorMsg = error instanceof Error ? error.message : "Unknown message send error";
      throw new MessageSendException(request.message.messageId, errorMsg);
    }
  }

  public async receiveMessage(request: ReceiveMessageRequest, context: IAgentExecutionContext): Promise<ReceiveMessageResult> {
    ...
    try {
      ...
      if (!registeredAgent) {
        throw new MessageReceiveException(request.agentId, "Agent not registered");
      }
      ...
      const errorMsg = error instanceof Error ? error.message : "Unknown message receive error";
      throw new MessageReceiveException(request.agentId, errorMsg);
    }
  }

  public async broadcastMessage(request: BroadcastMessageRequest, context: IAgentExecutionContext): Promise<BroadcastMessageResult> {
    ...
    try {
      ...
      this.failedMessages++;
      const errorMsg = error instanceof Error ? error.message : "Unknown message broadcast error";
      throw new MessageBroadcastException(request.message.messageId, errorMsg);
    }
  }

  public async subscribeToChannel(request: SubscribeChannelRequest, context: IAgentExecutionContext): Promise<SubscribeChannelResult> {
    const subscribedAt = new Date();
    
    try {
      // Validate channel ID
      if (!request.channelId || typeof request.channelId !== 'string') {
        throw new InvalidChannelException(request.channelId, "Channel ID must be a non-empty string");
      }
      
      // Create or get channel using existing manager
      const channel = await this.components.communicationManager.createChannel(request.channelId);
      ...
      if (!this.channelUsage[request.channelId]) {
        this.channelUsage[request.channelId] = { subscriptionCount: 0, messageCount: 0 };
      }
      ...
      const errorMsg = error instanceof Error ? error.message : "Unknown channel subscription error";
      throw new ChannelSubscriptionException(request.agentId, request.channelId, errorMsg);
    }
  }

  public async unsubscribeFromChannel(request: UnsubscribeChannelRequest, context: IAgentExecutionContext): Promise<UnsubscribeChannelResult> {
    ...
    try {
      ...
      const channels = await this.components.communicationManager.listChannels();
      const channelExists = channels.includes(request.channelId);
      
      if (!channelExists) {
        throw new InvalidChannelException(request.channelId, "Channel does not exist");
      }
      ...
      if (unsubscribed) {
        ...
        if (this.channelUsage[request.channelId]) {
          this.channelUsage[request.channelId].subscriptionCount = Math.max(0, this.channelUsage[request.channelId].subscriptionCount - 1);
      ...
      const errorMsg = error instanceof Error ? error.message : "Unknown channel unsubscription error";
      throw new ChannelUnsubscriptionException(request.agentId, request.channelId, errorMsg);
    }
  }
  ...
  public async getCommunicationAgentHealth(): Promise<CommunicationAgentHealth> {
    try {
      ...
      return {
        ...
        busAvailable: !!(this.components.communicationManager as any).getBus?.(),
        status,
  ...
  public getMetrics(): CommunicationAgentMetrics {
    ...
    const averageLatencies: Record<CommunicationOperation, number> = {} as Record<CommunicationOperation, number>;
    ...
    return {
      ...
      communicationStats: {
        ...
        totalSubscriptions: this.totalSubscriptions,
        totalUnsubscriptions: this.totalUnsubscriptions,
        sendSuccessRate: this.calculateOperationSuccessRate(CommunicationOperation.SEND),
        broadcastSuccessRate: this.calculateOperationSuccessRate(CommunicationOperation.BROADCAST),
        averageRecipientsPerBroadcast: this.totalBroadcasts > 0 ? this.broadcastRecipients / this.totalBroadcasts : 0,
        channelUsage: { ...this.channelUsage }
      }
    };
  }
  ...
  private validateAndParseRequest(input: unknown): CommunicationOperationRequest {
    if (!input || typeof input !== 'object') {
      throw new InvalidCommunicationOperationException("unknown", "Input must be an object");
    }
    
    const request = input as Record<string, unknown>;
    
    if (!request.operation || typeof request.operation !== 'string') {
      throw new InvalidCommunicationOperationException("unknown", "Operation is required and must be a string");
    }
    
    if (!Object.values(CommunicationOperation).includes(request.operation as CommunicationOperation)) {
      throw new InvalidCommunicationOperationException(request.operation as string, "Unsupported operation");
    }
    ...
    if (operation === CommunicationOperation.SEND) {
      if (!request.message || typeof request.message !== 'object') {
        throw new InvalidCommunicationOperationException(operation, "Message is required for send operation");
      }
    }
    
    if (operation === CommunicationOperation.RECEIVE) {
      if (!request.agentId || typeof request.agentId !== 'string') {
        throw new InvalidCommunicationOperationException(operation, "Agent ID is required for receive operation");
      }
    }
    
    if (operation === CommunicationOperation.BROADCAST) {
      if (!request.message || typeof request.message !== 'object') {
        throw new InvalidCommunicationOperationException(operation, "Message is required for broadcast operation");
      }
    }
    
    if ([CommunicationOperation.SUBSCRIBE, CommunicationOperation.UNSUBSCRIBE].includes(operation)) {
      if (!request.agentId || typeof request.agentId !== 'string') {
        throw new InvalidCommunicationOperationException(operation, "Agent ID is required for channel operations");
      }
      if (!request.channelId || typeof request.channelId !== 'string') {
        throw new InvalidCommunicationOperationException(operation, "Channel ID is required for channel operations");
      }
    }
  ...
  private validateMessage(message: AgentMessage): void {
    const requiredFields = ['messageId', 'senderAgentId', 'workspaceId', 'requestId', 'traceId', 'timestamp', 'priority', 'type', 'status', 'payload'];
    const missingFields = requiredFields.filter(field => !(field in message) || message[field as keyof AgentMessage] === undefined);
    ...
    if (typeof message.messageId !== 'string' || message.messageId.length === 0) {
      throw new InvalidMessageException(['messageId (must be non-empty string)']);
    }
    
    // Validate agent IDs
    if (typeof message.senderAgentId !== 'string' || message.senderAgentId.length === 0) {
      throw new InvalidMessageException(['senderAgentId (must be non-empty string)']);
    }
    
    // Validate workspace and request IDs
    if (typeof message.workspaceId !== 'string' || message.workspaceId.length === 0) {
      throw new InvalidMessageException(['workspaceId (must be non-empty string)']);
    }
    
    if (typeof message.requestId !== 'string' || message.requestId.length === 0) {
      throw new InvalidMessageException(['requestId (must be non-empty string)']);
    }
  ...
  private trackMessageStats(message: AgentMessage): void {
    // Track by type
    this.messagesByType[message.type] = (this.messagesByType[message.type] || 0) + 1;
    
    // Track by status
    this.messagesByStatus[message.status] = (this.messagesByStatus[message.status] || 0) + 1;
    
    // Track by priority
    this.messagesByPriority[message.priority] = (this.messagesByPriority[message.priority] || 0) + 1;
  }
  ...
  private recordOperationSuccess(operation: CommunicationOperation, duration: number): void {
  ...
  private recordOperationError(operation: CommunicationOperation, duration: number): void {
  ...
  private calculateOperationSuccessRate(operation: CommunicationOperation): number {
