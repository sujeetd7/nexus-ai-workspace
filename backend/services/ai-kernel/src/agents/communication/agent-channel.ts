...
export interface IAgentChannel<T = unknown> {
  ...
  subscribe(agentId: string, handler: MessageHandler<T>, filter?: (message: AgentMessage<T>) => boolean): Promise<string>;
...
export class AgentChannel<T = unknown> implements IAgentChannel<T> {
  ...
  private readonly subscriptions: Map<string, ChannelSubscription<T>> = new Map();
  ...
  public async publish(message: AgentMessage<T>): Promise<void> {
    ...
    const deliveryPromises = relevantSubscriptions.map(async (subscription) => {
      try {
        ...
        console.error(`Failed to deliver message to agent ${subscription.agentId}:`, error);
  ...
  public async getSubscribers(): Promise<string[]> {
    return Array.from(new Set(Array.from(this.subscriptions.values()).map(sub => sub.agentId)));
  }

  public async getSubscriptionsByAgent(agentId: string): Promise<ChannelSubscription<T>[]> {
  ...
  public async isSubscribed(agentId: string): Promise<boolean> {
    return Array.from(this.subscriptions.values()).some(sub => sub.agentId === agentId);
  }
