...
export interface IConversationMemory {
  ...
  history(context: MemoryContext, limit?: number): Promise<ConversationMessage[]>;
  last(context: MemoryContext, count?: number): Promise<ConversationMessage[]>;
  clear(context: MemoryContext): Promise<void>;
}

export class ConversationMemory extends BaseAgentMemory<ConversationMessage[]> implements IConversationMemory {
  private readonly maxHistorySize: number = 1000;

  public async append(message: ConversationMessage, context: MemoryContext): Promise<void> {
    try {
      ...
      if (messages.length > this.maxHistorySize) {
        ...
        this.warnings.push(`Conversation history truncated to ${this.maxHistorySize} messages`);
      }
  ...
  public async history(context: MemoryContext, limit?: number): Promise<ConversationMessage[]> {
    try {
      const conversationKey = this.getConversationKey(context);
  ...
  public async last(context: MemoryContext, count: number = 1): Promise<ConversationMessage[]> {
    try {
      const conversationKey = this.getConversationKey(context);
      const messages = await this.load(conversationKey, context) || [];
  ...
  public async getMessageById(messageId: string, context: MemoryContext): Promise<ConversationMessage | undefined> {
    try {
  ...
  public async getMessagesByRole(role: ConversationMessage['role'], context: MemoryContext): Promise<ConversationMessage[]> {
    try {
      const messages = await this.history(context);
