...
export class WorkspaceMemory<T = unknown>
  extends BaseAgentMemory<T>
  implements IWorkspaceMemory<T>
{
  ...
  public async put(
    ...
    try {
      ...
      // Check size limits
      if (
        !workspaceMap.has(key) &&
        workspaceMap.size >= this.maxKeysPerWorkspace
      ) {
        throw new Error(
          `Workspace memory limit reached: ${this.maxKeysPerWorkspace} keys`,
        );
      }
