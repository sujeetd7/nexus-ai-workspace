...
export interface ISharedMemory<T = unknown> {
  read(key: string, context: MemoryContext): Promise<SharedMemoryEntry<T> | undefined>;
  write(key: string, value: T, version: number, context: MemoryContext): Promise<boolean>;
...
export class SharedMemory<T = unknown> extends BaseAgentMemory<SharedMemoryEntry<T>> implements ISharedMemory<T> {
  private readonly locks: Map<string, { lockId: string; agentId: string; timestamp: Date }> = new Map();
  ...
  public async read(key: string, context: MemoryContext): Promise<SharedMemoryEntry<T> | undefined> {
    try {
  ...
  public async write(key: string, value: T, expectedVersion: number, context: MemoryContext): Promise<boolean> {
    try {
  ...
  public async unlock(key: string, lockId: string, context: MemoryContext): Promise<boolean> {
    try {
      const fullKey = this.buildKey(key, context);
      const lockInfo = this.locks.get(fullKey);
  ...
  public getLockInfo(key: string, context: MemoryContext): { lockId: string; agentId: string; timestamp: Date } | undefined {
    const fullKey = this.buildKey(key, context);
  ...
  private cleanupExpiredLocks(): void {
    ...
    for (const key of expiredKeys) {
      ...
      try {
        ...
        this.warnings.push(`Failed to clear expired lock info: ${error instanceof Error ? error.message : 'Unknown error'}`);
      }
    }
