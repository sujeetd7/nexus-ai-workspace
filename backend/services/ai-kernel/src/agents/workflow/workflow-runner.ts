...
export class WorkflowRunner implements IWorkflowRunner {
  ...
  public async runSequential(stepIds: string[], execution: WorkflowExecution): Promise<void> {
    for (const stepId of stepIds) {
      ...
      try {
        ...
        const stepExecution = await this.executor.executeStep(stepId, execution);
        ...
        if (stepExecution.state === WorkflowState.COMPLETED) {
          ...
          if (!this.shouldContinueOnFailure(execution)) {
            throw new WorkflowStepException(stepId, execution.executionId, "sequential");
          }
        }
  ...
  public async runParallel(stepIds: string[], execution: WorkflowExecution): Promise<void> {
  ...
  public async runConditional(
    ...
    try {
      const conditionResult = await this.executor.evaluateCondition(condition, execution.context);
  ...
  public async runLoop(loopConfig: WorkflowLoop, steps: string[], execution: WorkflowExecution): Promise<void> {
    let iterations = 0;
    ...
    try {
      while (iterations < maxIterations) {
        ...
        try {
          shouldContinue = await this.evaluateLoopCondition(loopConfig, execution);
        } catch (error) {
          throw new WorkflowConditionException(
  ...
  public async runCompensation(stepIds: string[], execution: WorkflowExecution): Promise<void> {
    ...
    for (const stepId of reversedStepIds) {
      try {
        ...
        execution.metadata.compensationErrors = execution.metadata.compensationErrors || [];
  ...
  private async evaluateLoopCondition(loopConfig: WorkflowLoop, execution: WorkflowExecution): Promise<boolean> {
    const condition = loopConfig.condition;
    
    switch (loopConfig.type) {
      case "while":
        return await this.executor.evaluateCondition(condition.expression, execution.context);
        
      case "for":
        const currentIteration = execution.context.variables.loopIteration as number || 0;
        const maxIterations = loopConfig.maxIterations;
        return currentIteration < maxIterations;
        
      case "forEach":
        // For forEach, check if there are remaining items in the collection
        const collection = execution.context.variables[condition.variables[0]] as unknown[];
        const forEachIndex = execution.context.variables.forEachIndex as number || 0;
  ...
  private shouldContinueOnFailure(execution: WorkflowExecution): boolean {
    ...
    const continueOnError = execution.metadata.continueOnError as boolean || false;
