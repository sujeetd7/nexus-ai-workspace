...
export class UsageService {
  ...
  async trackUsage(data: UsageData): Promise<UsageResult> {
    ...
    console.log(`[USAGE] ${data.provider}/${data.model} - ${data.totalTokens} tokens - $${estimatedCost.toFixed(6)} - ${data.latency}ms`, {
  ...
  private calculateCost(data: UsageData): number {
    ...
    const inputCost = (data.promptTokens / 1000) * modelConfig.costPer1kTokens.input;
    const outputCost = (data.completionTokens / 1000) * modelConfig.costPer1kTokens.output;

    return inputCost + outputCost;
  }

  async getUsageStats(workspaceId?: string, userId?: string, timeframe?: {
    start: Date;
    end: Date;
  }): Promise<{
    totalCost: number;
  ...
  async getTopModels(workspaceId?: string, limit: number = 10): Promise<Array<{
    provider: string;
    model: string;
    totalTokens: number;
    totalCost: number;
  ...
  async getCostProjection(workspaceId?: string, projectionDays: number = 30): Promise<{
