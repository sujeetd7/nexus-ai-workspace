...
1. Critical blockers (mocks, gateway ports/auth, invitation, kernel Prisma/MCP decision)
2. Wiring (validation, swagger, mounts, route order)
3. Delete or quarantine dead trees (nested workspace, unused agents, stub gateway)
4. Per-service hardening (DI optional later; fail-closed + auth first)
